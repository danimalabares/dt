...
\begin{proof}[Solution]\leavevmode
	...
	Now let's use Exercise \hyperref[exer:2.29]{2.29}. We can separate \(K_1:=\partial U\) and \(K_2:=U \setminus V\). Both are compact, and they are disjoint because the boundary of \(U\) is disjoint from \(U\), so it must be contained in \(V\). Then there are disjoint neighbourhoods \(U_1\) and \(U_2\) of \(K_1\) and \(K_2\).
