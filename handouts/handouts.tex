...
\begin{minipage}{\textwidth}
	...
	Handouts by Misha Verbitsky	 \hfill Daniel González Casanova Azuela
		...
		{\small \href{http://verbit.ru/MATH/GEOM-2013/}{verbit.ru/MATH/GEOM-2013} \hfill\href{https://github.com/danimalabares/dt}{github.com/danimalabares/dt}}
	\end{minipage}
...
\begin{thing4}{Exercise 1.17}\leavevmode
...
is open. Now $\pi^{-1}(\pi(F))$ is the set of points that are $G$-related to points in  $F$, namely $\bigcup_{g \in G} gF$. Since $G$ is finite and  acts by homeomorphisms, this set is a finite union of closed sets, which is closed. {\color{2}Looks like I didn't use the Hausdorff hypothesis.}
\end{proof}
...
\begin{proof}[Solution]\leavevmode
If we show that the diagonal is closed in $ M \times M$, then its complement would be an open set, and when projecting to $(M/\sim) \times (M/\sim)$ we obtain that the diagonal is closed in the latter space.
\end{proof}
\end{document}
