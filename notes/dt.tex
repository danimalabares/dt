...
\item Never forget that the Möbius strip is not orientable because the \textit{\textbf{mod 2}} self-intersection number of the ``soul" circle is not zero, and the \textit{\textbf{oriented}} intersection number of a submanifold \(X\) of is \(I(X,X)=(-1)^{-\dim X}I(X,X)\). (So it would be zero if Möbius was oriented.)
\end{enumerate}

\section{Aula 14: Teorema de Poincaré-Hopf}
